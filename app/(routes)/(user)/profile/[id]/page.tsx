...
function BrowseUserProfile({ params }) {
    ...
    const handleGetProfileData = async () => {
        ...
        try {
            const { user, articles } = await fetchUserProfileAndArticles(userId);
        ...
        <div className="container lg:max-w-[1000px] xl:max-w-[1200px] 2xl:max-w-[1400px] mx-auto py-10">
            ...
            {userInfo && (
                <>
                    ...
                    {/* User Articles */}
                    <>
                        <h1 className="text-center mb-8 capitalize">{userInfo.name}'s Articles</h1>
                        <div className='my-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-5 gap-y-10 items-stretch justify-center'>
                            {userArticles?.map((article, index) => (
                                <div key={index}>
                                    <Card sx={{ maxWidth: 345 }} className="!rounded-main !h-full mx-auto !shadow-sm hover:!shadow-lg hover:!shadow-shadows">
                                        <CardMedia
                                            sx={{ height: 180 }}
                                            image={article.thumbnail || '/public/assets/images/full-back-article.jpeg'}
                                        ...
                                        <div className="h-[calc(100%-180px)] flex flex-col !items-between">
                                            ...
                                            <CardActions className='justify-between !px-4 !mt-auto'>
                                                ...
                                                <p className="ms-auto">{new Date(article.createdAt as string).toLocaleDateString()}</p>
                                            </CardActions>
                                        </div>
                                    </Card>
                                </div>
                        ...
                        {userArticles?.length === 0 && <p className="my-5 bg-white p-5 font-medium rounded-main shadow">Didn't publish any articles yet.</p>}
                    </>
                </>
            )}
        </div>
